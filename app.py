...
MAX_CONTEXT_TOKENS = 12000 # Límite de tokens para el contexto enviado a GPT-4o (GPT-4o tiene 128k, pero usamos un límite seguro)
...
def normalize_text(text):
    """
    Convierte el texto a minúsculas y elimina los acentos para facilitar las comparaciones.
    """
    if not text:
        return ""
    text = text.lower()
...
def get_question_intent(question):
    """
    Determina la intención de la pregunta para decidir cómo responder.
    """
    normalized_q = normalize_text(question)

    # 1. Intención de Respuesta Directa (no requiere LLM)
    ...
    # 2. Intención de Saludo o Conversación Simple
    ...
    # 3. Intención Irrelevante (temas fuera del ámbito académico)
    irrelevant_keywords = [
    ...
    # 4. Intención de Conocimiento General
    general_keywords = [
    ...
    # 5. Por defecto, se asume que es una pregunta sobre los documentos.
...
def load_and_chunk_documents():
    """
    Carga todos los PDFs, los divide en fragmentos (chunks) y los devuelve.
    """
    all_chunks = []
...
def get_relevant_chunks(chunks, question):
    """
    Encuentra los fragmentos más relevantes para una pregunta usando TF-IDF.
    """
...
def get_openai_response(system_prompt, user_prompt, chat_history):
    """
    Envía la solicitud a la API de OpenAI y devuelve la respuesta.
    """
...
def validar_estudiante(codigo):
    """
    Valida el código del estudiante contra un archivo Excel.
    """
    try:
        ...
        if not os.path.exists(excel_path):
            ...
            return False, None
        df = pd.read_excel(excel_path, usecols=['codigo', 'nombre', 'apellido'])
...
def main():
    """
    Función principal que ejecuta la interfaz de usuario de Streamlit.
    """
    ...
    if not st.session_state.authenticated:
        ...
        with st.form("login_form"):
            ...
            if submitted:
                is_valid, student_name = validar_estudiante(codigo)
                if is_valid:
            ...
            with st.spinner("Pensando..."):
                ...
                if intent == "direct_answer" or intent == "irrelevant":
                    ...
                    if not relevant_chunks:
                        ...
                        # Construir contexto respetando el límite de tokens
                        context_text = ""
                        prompt_tokens = len(ENCODER.encode(prompt))
                        # Dejar un buffer de 1500 tokens para la respuesta y el prompt del sistema base
